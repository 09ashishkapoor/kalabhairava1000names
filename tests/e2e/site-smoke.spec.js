...
test('loads the landing page and initial names explorer content', async ({ page, baseURL }) => {
...
test('supports focused search on the bootstrap dataset without breaking the reader', async ({ page, baseURL }) => {
  const runtimeGuards = await installRuntimeGuards(page, baseURL);
  ...
  await expect.poll(async () => page.locator('#names-grid .name-card').count()).toBeGreaterThan(0);
  await expect(page.locator('#names-grid .name-card').first()).toContainText('Bhairavaaya');
...
test('has no critical accessibility violations in the names explorer flow', async ({ page, baseURL }) => {
  const runtimeGuards = await installRuntimeGuards(page, baseURL);
